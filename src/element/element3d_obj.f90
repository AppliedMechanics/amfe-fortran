...
module element3d_obj
    use, intrinsic:: iso_fortran_env, only: dp=>real64
    use shapefunction_obj
    use integrand_structural3d_obj
    use material_obj
    implicit none
    ...
    type element3d
        ...
        contains
            procedure :: f_int_and_jac => element3d_f_int_and_jac

    end type element3d

    contains

        subroutine element3d_f_int_and_jac(this, out, xn, d, dd, t, s0, t0, s1)
            implicit none
            class(element3d) :: this ! type bound procedure (not object bounded), thus 'class' instead of 'type'
            ...
            real(dp) :: dn_dxi(this%shapefunction%no_of_nodes,3)
            ...
            do g=1,no_of_gps
                ...
                ! ---------------------------- Call shapefunctions --------------------------------------------
                call this%shapefunction%evaluate(this%gausspoints_kf_points(1,g), &
                        this%gausspoints_kf_points(2,g), &
                        this%gausspoints_kf_points(3,g), n)


                ! ---------------------------- Call weak form -------------------------------------------------
                call this%integrand%f_int_and_jac(n, dn_dxi, xn, d, dd, t, matfunc3d_internal, s0_local, t0, s1_local, &
                f_int_k_dk_gp, this%integrand%no_of_fields(), 3, this%shapefunction%no_of_nodes, no_of_states_per_gp)
            ...
            contains
                subroutine matfunc3d_internal(f, df, t, s0, t0, s1, sv_out, c_out, dc_out)
                ...
                call this%material%sv_c_dc_3d(f, df, t, s0, t0, s1, sv_out, c_out, dc_out)

                end subroutine matfunc3d_internal

        end subroutine element3d_f_int_and_jac

end module element3d_obj
